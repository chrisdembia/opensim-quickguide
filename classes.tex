...
%        every node/.style={draw}, % TODO for debugging.
    ...
    % Reporters.
    \node (AbstractReporter) [abstract, below right=3ex and 0em of Component]
    {AbstractReporter};
    ...
    \begin{scope} [
        ...
        \node (ConsoleReporter) [below=of TableReporter.west,
            anchor=west]
            {ConsoleReporter};
        \node (StatesAccumulator) [below=of ConsoleReporter.west,
            anchor=west]
            {StatesAccumulator};
        \draw[<-] (AbstractReporter) |- (TableReporter);
        \draw[<-] (AbstractReporter) |- (ConsoleReporter);
        \draw[<-] (AbstractReporter) |- (StatesAccumulator);

    \end{scope}

    % TableSource.
    \node (TableSource) [right=4em of AbstractReporter] {TableSource};
    ...
    % Tool.
    \node (Tool) [abstract, below right=3ex and 26em of Object] {Tool};
    \node (AbstractTool) [abstract, right=7em of Tool] {AbstractTool};
    ...
    \begin{scope} [
        ...
        \node (MOTFileAdapter) [below=of CSVFileAdapter.west, anchor=west]
        {MOTFileAdapter};
        \node (TRCFileAdapter) [below=of MOTFileAdapter.west, anchor=west]
        {TRCFileAdapter};
        \node (C3DFileAdapter) [below=of TRCFileAdapter.west, anchor=west]
        {C3DFileAdapter};

        \draw [<-] (DataAdapter) |- (CSVFileAdapter);
        \draw [<-] (DataAdapter) |- (MOTFileAdapter);
